...
try:
    total_data = pd.DataFrame(columns=['atclNm', 'tradTpNm', 'flrInfo', 'prc', 'hanPrc', 'spc1', 'spc2', 'lat', 'lng', 'tagList', '지역구'])

    for url_guname, url_data in url_df.values:
        page = 1
        atclNm = []
        tradTpNm = []
        flrInfo = []
        ...
        while True:
            ...
            temp_data = pd.DataFrame(
                (atclNm, tradTpNm, flrInfo, prc, hanPrc,rentPrc, spc1, spc2, lat, lng, tagList)).transpose()
            temp_data.columns = ['atclNm', 'tradTpNm', 'flrInfo', 'prc', 'hanPrc',"rentPrc", 'spc1', 'spc2', 'lat', 'lng', 'tagList']
            temp_data['지역구'] = url_guname
